...
Simulation::Simulation(int width, int height, std::unique_ptr<Canvas> canvas, std::unique_ptr<StatisticsHandler> sh) : _sim_width{width}, _sim_height{height}, _canvas{std::move(canvas)}, _sh{std::move(sh)} {}
...
void Simulation::tick()
{
      ...
      if (tick_count % 30 == 0)
      {
            _sh.get()->communicate_number_infected(tick_count / 30, numberInfected);
      }
...
void Simulation::subject_collision(Subject &s1, Subject &s2)
{
      ...
      if (dist < s1.radius() + s2.radius())
      {
            ...
            if (s1.infected())
            {
                  s2.infect(tick_count);
            }

            if (s2.infected())
            {
                  s1.infect(tick_count);
            }
