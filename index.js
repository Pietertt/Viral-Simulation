...
var STATIC_BASE = 1024,
    ...
    STACKTOP = STACK_BASE,
    STACK_MAX = 26320,
    DYNAMIC_BASE = 5269200,
    DYNAMICTOP_PTR = 26160;
...
var ASM_CONSTS = {
  1024: function($0, $1) {var config = window.myConfig; var myLine = window.myLine; if (config.data.datasets.length > 0) { config.data.labels.push($0); config.data.datasets.forEach(function(dataset) { dataset.data.push($1); }); window.myLine.update(); }},  
 1432: function($0, $1, $2, $3, $4) {var canvas = document.createElement('canvas'); canvas.id = Module.AsciiToString($0); canvas.style.position = 'absolute'; canvas.style.left = $1 + "px"; canvas.style.top = $2 + "px"; canvas.style.width = $3 + "px"; canvas.style.height = $4 + "px"; canvas.width = $3; canvas.height = $4; document.body.append(canvas); var context = canvas.getContext('2d'); window[canvas.id] = context;},  
 1848: function($0, $1, $2) {var context = window[Module.AsciiToString($0)]; context.clearRect(0,0,$1,$2);},  
 1934: function($0, $1, $2, $3, $4, $5) {var context = window[Module.AsciiToString($0)]; context.fillStyle = Module.AsciiToString($1); context.fillRect($2, $3, $4, $5);},  
 2073: function($0, $1, $2, $3, $4) {var context = window[Module.AsciiToString($0)]; context.fillStyle = Module.AsciiToString($1); context.beginPath(); context.arc($2, $3, $4, 0, Math.PI * 2, true); context.fill();}
};
...
var asmGlobalArg = {};
var asmLibraryArg = { "__cxa_allocate_exception": ___cxa_allocate_exception, "__cxa_atexit": ___cxa_atexit, "__cxa_throw": ___cxa_throw, "__handle_stack_overflow": ___handle_stack_overflow, "__map_file": ___map_file, "__sys_munmap": ___sys_munmap, "__sys_open": ___sys_open, "__sys_read": ___sys_read, "abort": _abort, "emscripten_asm_const_iii": _emscripten_asm_const_iii, "emscripten_get_sbrk_ptr": _emscripten_get_sbrk_ptr, "emscripten_memcpy_big": _emscripten_memcpy_big, "emscripten_resize_heap": _emscripten_resize_heap, "emscripten_sleep": _emscripten_sleep, "environ_get": _environ_get, "environ_sizes_get": _environ_sizes_get, "fd_close": _fd_close, "fd_read": _fd_read, "fd_seek": _fd_seek, "fd_write": _fd_write, "memory": wasmMemory, "setTempRet0": _setTempRet0, "strftime_l": _strftime_l, "table": wasmTable };
